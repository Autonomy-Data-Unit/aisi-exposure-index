...
# %% [markdown]
# # nodes.pass5_seniority_management
#
# Pass 5: classify the advertised role's seniority level and management
# responsibility. Independent of the AI keyword filter — Pass 5 is "seniority
# for all ads" and is gated by its own `pass5_route` (defaulting to "full").
#
# Reads:
# - `routing/annotation_universe_v1.parquet` for title and description.
# - `routing/annotation_routing_v1.parquet` for `eligible_pass5`.
#
# Writes:
# - `batch1_core_salience_seniority/pass5_results.duckdb` (ResultStore).
# - `batch1_core_salience_seniority/pass5_seniority_management_v1.parquet`.
# - `batch1_core_salience_seniority/pass5_seniority_management_v1_failures.parquet`.
# - `batch1_core_salience_seniority/pass5_meta.json`.

# %%
#|default_exp pass5_seniority_management
#|export_as_func true
...
async def main(ctx, print, ad_ids: list[int]) -> {
    ...
    """Run Pass 5 (seniority + management) on eligible ads."""
    ...

# %%
from dev_utils import *
run_name = 'annotation_smoke_5k'
set_node_func_args('pass5_seniority_management', run_name=run_name)
show_node_vars('pass5_seniority_management', run_name=run_name)
...
from ai_annotations.schemas import SeniorityManagementV1
...
db_path = output_dir / "pass5_results.duckdb"
out_parquet_path = output_dir / "pass5_seniority_management_v1.parquet"
failures_path = output_dir / "pass5_seniority_management_v1_failures.parquet"
meta_path = output_dir / "pass5_meta.json"

print(f"pass5: model={llm_model}, structured_output={_use_structured_output}, reasoning={_is_reasoning}")
...
# %%
#|export
_routing = pq.read_table(routing_path, columns=["ad_id", "eligible_pass5"])
_routing_df = _routing.to_pandas()
_eligible_set = set(int(x) for x in _routing_df[_routing_df["eligible_pass5"]]["ad_id"].tolist())

eligible_ad_ids = [int(aid) for aid in ad_ids if int(aid) in _eligible_set]
print(f"pass5: {len(eligible_ad_ids)} / {len(ad_ids)} ads eligible for Pass 5")
...
print(f"pass5: built {len(ad_id_to_prompt)} prompts")
...
#|export
_slurm_jobs = []
_schema_json = SeniorityManagementV1.model_json_schema() if _use_structured_output else None
    ...
    for aid, response in zip(chunk_ids, responses):
        try:
            if _is_reasoning or not _use_structured_output:
                parsed = extract_json(response, validator=SeniorityManagementV1.model_validate)
                if parsed is None:
                ...
                parsed = SeniorityManagementV1.model_validate_json(response)
                stored = parsed.model_dump_json()
...
if eligible_ad_ids:
    pass5_meta = await run_batched(
        eligible_ad_ids, store, _work_fn,
        batch_size=chunk_size,
        max_concurrent=max_concurrent,
        max_retries=max_retries,
        resume=resume,
        node_name="pass5_seniority_management",
        print_fn=print,
        raise_on_failure=raise_on_failure,
    )
else:
    pass5_meta = {"n_attempted": 0, "n_success": 0, "n_failed": 0, "failed_ids": []}

store.close()
del store
pass5_meta["slurm_jobs"] = _slurm_jobs
pass5_meta["slurm_total_seconds"] = sum(j.get("elapsed_seconds", 0) for j in _slurm_jobs)
pass5_meta["model_name"] = llm_model
pass5_meta["prompt_version"] = prompt_version
...
print(f"pass5: {len(success_rows)} successes, {len(failure_rows)} failures")
...
if _success_records:
    ...
    print(f"pass5: wrote {const.rel(out_parquet_path)}")
else:
    empty_schema = pa.schema([
        ("ad_id", pa.int64()),
        ("seniority_level", pa.string()),
        ("management_level", pa.string()),
        ("seniority_management_confidence", pa.float64()),
        ("seniority_management_evidence", pa.string()),
    ...
    print(f"pass5: wrote empty success parquet at {const.rel(out_parquet_path)}")
...
if _failure_records:
    ...
    print(f"pass5: wrote {const.rel(failures_path)}")
else:
...
with open(meta_path, "w") as f:
    json.dump(pass5_meta, f, indent=2)
print(f"pass5: wrote {const.rel(meta_path)}")
