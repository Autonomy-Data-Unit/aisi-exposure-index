...
# %% [markdown]
# # nodes.pass6_quality_remote_agency
#
# Pass 6: extract data-quality and work-location signals from the ad:
# `job_text_informativeness`, `remote_status`, `recruitment_agency_likely`.
# This is the robustness/quality-control layer rather than an analytic
# headline; the spec allows running Pass 6 on `full` even when the rest of
# Batch 3 is `post_filter` (via `pass6_route_override`).
#
# Eligibility: `routing/annotation_routing_v1.parquet` column `eligible_pass6`.

# %%
#|default_exp pass6_quality_remote_agency
#|export_as_func true
...
async def main(ctx, print, ad_ids: list[int]) -> {
    ...
    """Run Pass 6 (informativeness + remote status + agency) on eligible ads."""
    ...

# %%
from dev_utils import *
run_name = 'annotation_smoke_5k'
set_node_func_args('pass6_quality_remote_agency', run_name=run_name)
show_node_vars('pass6_quality_remote_agency', run_name=run_name)
...
from ai_annotations.schemas import QualityRemoteAgencyV1
...
output_dir = annotations_dir / "batch3_worker_use_quality"
output_dir.mkdir(parents=True, exist_ok=True)

universe_path = routing_dir / "annotation_universe_v1.parquet"
routing_path = routing_dir / "annotation_routing_v1.parquet"
db_path = output_dir / "pass6_results.duckdb"
out_parquet_path = output_dir / "pass6_quality_remote_agency_v1.parquet"
failures_path = output_dir / "pass6_quality_remote_agency_v1_failures.parquet"
meta_path = output_dir / "pass6_meta.json"

print(f"pass6: model={llm_model}, structured_output={_use_structured_output}, reasoning={_is_reasoning}")
...
# %%
#|export
_routing = pq.read_table(routing_path, columns=["ad_id", "eligible_pass6"])
_routing_df = _routing.to_pandas()
_eligible_set = set(int(x) for x in _routing_df[_routing_df["eligible_pass6"]]["ad_id"].tolist())

eligible_ad_ids = [int(aid) for aid in ad_ids if int(aid) in _eligible_set]
print(f"pass6: {len(eligible_ad_ids)} / {len(ad_ids)} ads eligible for Pass 6")
...
print(f"pass6: built {len(ad_id_to_prompt)} prompts")
...
#|export
_slurm_jobs = []
_schema_json = QualityRemoteAgencyV1.model_json_schema() if _use_structured_output else None
    ...
    for aid, response in zip(chunk_ids, responses):
        try:
            if _is_reasoning or not _use_structured_output:
                parsed = extract_json(response, validator=QualityRemoteAgencyV1.model_validate)
                if parsed is None:
                ...
                parsed = QualityRemoteAgencyV1.model_validate_json(response)
                stored = parsed.model_dump_json()
...
if eligible_ad_ids:
    pass6_meta = await run_batched(
        eligible_ad_ids, store, _work_fn,
        batch_size=chunk_size,
        max_concurrent=max_concurrent,
        max_retries=max_retries,
        resume=resume,
        node_name="pass6_quality_remote_agency",
        print_fn=print,
        raise_on_failure=raise_on_failure,
    )
else:
    pass6_meta = {"n_attempted": 0, "n_success": 0, "n_failed": 0, "failed_ids": []}

store.close()
del store
pass6_meta["slurm_jobs"] = _slurm_jobs
pass6_meta["slurm_total_seconds"] = sum(j.get("elapsed_seconds", 0) for j in _slurm_jobs)
pass6_meta["model_name"] = llm_model
pass6_meta["prompt_version"] = prompt_version
...
print(f"pass6: {len(success_rows)} successes, {len(failure_rows)} failures")
...
if _success_records:
    ...
    print(f"pass6: wrote {const.rel(out_parquet_path)}")
else:
    empty_schema = pa.schema([
        ("ad_id", pa.int64()),
        ("job_text_informativeness", pa.string()),
        ("remote_status", pa.string()),
        ("recruitment_agency_likely", pa.bool_()),
        ("data_quality_confidence", pa.float64()),
        ("data_quality_evidence", pa.string()),
    ...
    print(f"pass6: wrote empty success parquet at {const.rel(out_parquet_path)}")
...
if _failure_records:
    ...
    print(f"pass6: wrote {const.rel(failures_path)}")
else:
...
with open(meta_path, "w") as f:
    json.dump(pass6_meta, f, indent=2)
print(f"pass6: wrote {const.rel(meta_path)}")
