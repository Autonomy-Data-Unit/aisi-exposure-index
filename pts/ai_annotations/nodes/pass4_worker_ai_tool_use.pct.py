...
# %% [markdown]
# # nodes.pass4_worker_ai_tool_use
#
# Pass 4: classify whether the advertised worker is expected to use AI tools
# in their ordinary work (`worker_ai_tool_use_level`) and the main task area
# (`worker_ai_tool_use_area`).
#
# Eligibility: `routing/annotation_routing_v1.parquet` column `eligible_pass4`
# (Pass-1-positive under post_filter).

# %%
#|default_exp pass4_worker_ai_tool_use
#|export_as_func true
...
async def main(ctx, print, ad_ids: list[int]) -> {
    ...
    """Run Pass 4 (worker AI tool use + task area) on eligible ads."""
    ...

# %%
from dev_utils import *
run_name = 'annotation_smoke_5k'
set_node_func_args('pass4_worker_ai_tool_use', run_name=run_name)
show_node_vars('pass4_worker_ai_tool_use', run_name=run_name)
...
from ai_annotations.schemas import WorkerAIToolUseV1
...
db_path = output_dir / "pass4_results.duckdb"
out_parquet_path = output_dir / "pass4_worker_ai_tool_use_v1.parquet"
failures_path = output_dir / "pass4_worker_ai_tool_use_v1_failures.parquet"
meta_path = output_dir / "pass4_meta.json"

print(f"pass4: model={llm_model}, structured_output={_use_structured_output}, reasoning={_is_reasoning}")
...
# %%
#|export
_routing = pq.read_table(routing_path, columns=["ad_id", "eligible_pass4"])
_routing_df = _routing.to_pandas()
_eligible_set = set(int(x) for x in _routing_df[_routing_df["eligible_pass4"]]["ad_id"].tolist())

eligible_ad_ids = [int(aid) for aid in ad_ids if int(aid) in _eligible_set]
print(f"pass4: {len(eligible_ad_ids)} / {len(ad_ids)} ads eligible for Pass 4")
...
print(f"pass4: built {len(ad_id_to_prompt)} prompts")
...
#|export
_slurm_jobs = []
_schema_json = WorkerAIToolUseV1.model_json_schema() if _use_structured_output else None
    ...
    for aid, response in zip(chunk_ids, responses):
        try:
            if _is_reasoning or not _use_structured_output:
                parsed = extract_json(response, validator=WorkerAIToolUseV1.model_validate)
                if parsed is None:
                ...
                parsed = WorkerAIToolUseV1.model_validate_json(response)
                stored = parsed.model_dump_json()
...
if eligible_ad_ids:
    pass4_meta = await run_batched(
        eligible_ad_ids, store, _work_fn,
        batch_size=chunk_size,
        max_concurrent=max_concurrent,
        max_retries=max_retries,
        resume=resume,
        node_name="pass4_worker_ai_tool_use",
        print_fn=print,
        raise_on_failure=raise_on_failure,
    )
else:
    pass4_meta = {"n_attempted": 0, "n_success": 0, "n_failed": 0, "failed_ids": []}

store.close()
del store
pass4_meta["slurm_jobs"] = _slurm_jobs
pass4_meta["slurm_total_seconds"] = sum(j.get("elapsed_seconds", 0) for j in _slurm_jobs)
pass4_meta["model_name"] = llm_model
pass4_meta["prompt_version"] = prompt_version
...
print(f"pass4: {len(success_rows)} successes, {len(failure_rows)} failures")
...
if _success_records:
    ...
    print(f"pass4: wrote {const.rel(out_parquet_path)}")
else:
    empty_schema = pa.schema([
        ("ad_id", pa.int64()),
        ("worker_ai_tool_use_level", pa.string()),
        ("worker_ai_tool_use_area", pa.string()),
        ("worker_ai_tool_use_confidence", pa.float64()),
        ("worker_ai_tool_use_evidence", pa.string()),
    ...
    print(f"pass4: wrote empty success parquet at {const.rel(out_parquet_path)}")
...
if _failure_records:
    ...
    print(f"pass4: wrote {const.rel(failures_path)}")
else:
...
with open(meta_path, "w") as f:
    json.dump(pass4_meta, f, indent=2)
print(f"pass4: wrote {const.rel(meta_path)}")
